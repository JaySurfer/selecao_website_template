    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-6 sm:gap-8">
        ...
        >
          ...
          >
            <div class="opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 ease-in-out">
              ...
              <div class="flex space-x-3">
                <a :href="item.lightboxUrl || item.imageUrl" target="_blank" class="w-8 h-8 sm:w-9 sm:h-9 bg-white/20 hover:bg-white/40 rounded-full flex items-center justify-center text-white transition-colors">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4 sm:w-5 sm:h-5"><path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" /></svg>
                </a>
                <a :href="item.detailsUrl || '#'" target="_blank" class="w-8 h-8 sm:w-9 sm:h-9 bg-white/20 hover:bg-white/40 rounded-full flex items-center justify-center text-white transition-colors">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4 sm:w-5 sm:h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 0 1 0 6.624l-5.173 5.173a4.5 4.5 0 0 1-6.363-6.363l.507-.507M10.81 15.312a4.5 4.5 0 0 1 0-6.624l5.173-5.173a4.5 4.5 0 0 1 6.363 6.363l-.507.507" /></svg>
                </a>
              </div>
            </div>
          </div>
...
import imgSunglasses from '@/assets/images/camera-lens-portfolio-7.jpg'; // Placeholder, update if you have a sunglasses image
import imgLens from '@/assets/images/camera-lens-portfolio-7.jpg'; // Using lens for both, update if needed
...
const portfolioItems = ref<PortfolioItem[]>([
  { id: 1, imageUrl: imgCurology, title: 'Curology', category: 'Product', categoryText: 'Skincare', layoutHint: 'wide' },
  { id: 2, imageUrl: imgTeaHoney, title: 'Tea & Honey', category: 'App', categoryText: 'Lifestyle', layoutHint: 'normal' },
  // Tall items next
  { id: 4, imageUrl: imgStool, title: 'Minimal Stool', category: 'Branding', categoryText: 'Furniture', layoutHint: 'tall' }, // Stool first of the talls
  { id: 5, imageUrl: imgLamp, title: 'Desk Lamp', category: 'Product', categoryText: 'Lighting', layoutHint: 'tall' },   // Lamp next
  // Normal items that would appear next to the tall ones
  { id: 3, imageUrl: imgSucculent, title: 'Succulent', category: 'Web', categoryText: 'Decor', layoutHint: 'normal' },
  { id: 7, imageUrl: imgSunglasses, title: 'Sunglasses', category: 'App', categoryText: 'Fashion', layoutHint: 'normal' }, // Placeholder for sunglasses
  // Watch - might be shorter, wider
  { id: 6, imageUrl: imgWatch, title: 'Classic Watch', category: 'Branding', categoryText: 'Accessory', layoutHint: 'short-wide' },
  // Remaining normal items
  { id: 8, imageUrl: imgLens, title: 'Camera Lens', category: 'Web', categoryText: 'Photography', layoutHint: 'normal' },
  { id: 9, imageUrl: imgGlass, title: 'Artisan Drink', category: 'Product', categoryText: 'Beverage', layoutHint: 'normal' }
]);
...
const filteredPortfolioItems = computed(() => {
  ...
  return portfolioItems.value.filter(item =>
    mapCategoryToFilter(item.category).toLowerCase() === activeFilter.value.toLowerCase()
  );
});
